...
export default function ProgressSection({ profile, entries }: Props) {
  ...
  return (
    <div className="space-y-6">
      {/* AI Scenarios */}
      <AIScenarios profile={profile} entries={entries} />
      {/* Main Progress Bar */}
      <div className="glass p-4 sm:p-6 animate-slide-up">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="text-base sm:text-lg font-semibold text-gray-900 dark:text-gray-100">Celkový pokrok</h3>
            <p className="text-xs sm:text-sm text-gray-600 dark:text-gray-400">
              {profile.startWeight} kg &rarr; {profile.targetWeight} kg
            </p>
          </div>
          <div className="text-right">
            <div className="text-2xl sm:text-3xl font-bold text-emerald-500 dark:text-emerald-400">{percentComplete.toFixed(1)}%</div>
            <div className="text-xs text-gray-600 dark:text-gray-400">dokončeno</div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-2 sm:gap-4 text-center">
          <div>
            <div className="text-sm sm:text-lg font-bold text-emerald-500 dark:text-emerald-400">-{totalLost.toFixed(1)} kg</div>
            <div className="text-xs text-gray-600 dark:text-gray-400">Shozeno</div>
          </div>
          <div>
            <div className="text-sm sm:text-lg font-bold text-gray-700 dark:text-gray-300">{latestWeight.toFixed(1)} kg</div>
            <div className="text-xs text-gray-600 dark:text-gray-400">Aktuální</div>
          </div>
          <div>
            <div className="text-sm sm:text-lg font-bold text-amber-500 dark:text-amber-400">{remaining.toFixed(1)} kg</div>
            <div className="text-xs text-gray-600 dark:text-gray-400">Zbývá</div>
          </div>
        </div>
      </div>

      {/* Calorie Info */}
      <div className="glass p-4 sm:p-6 animate-slide-up" style={{ animationDelay: "0.1s" }}>
        <h3 className="text-base sm:text-lg font-semibold text-gray-900 dark:text-gray-100 mb-4">Kalorický plán</h3>
        <div className="grid grid-cols-2 gap-3 sm:gap-4">
          <div className="stat-card p-3 sm:p-4">
            <span className="stat-label">BMR</span>
            <span className="stat-value text-lg sm:text-xl">{calorieInfo.bmr}</span>
            <span className="text-xs text-gray-600 dark:text-gray-400">kcal/den</span>
          </div>
          <div className="stat-card p-3 sm:p-4">
            <span className="stat-label">TDEE</span>
            <span className="stat-value text-lg sm:text-xl">{calorieInfo.tdee}</span>
            <span className="text-xs text-gray-600 dark:text-gray-400">kcal/den</span>
          </div>
          <div className="stat-card p-3 sm:p-4 col-span-2 bg-emerald-500/5 border-emerald-500/20">
            <span className="stat-label text-emerald-500">Denní příjem (deficit {calorieInfo.deficit} kcal)</span>
            <span className="stat-value text-2xl sm:text-3xl text-emerald-500 dark:text-emerald-400">{calorieInfo.dailyTarget}</span>
            <span className="text-xs text-gray-600 dark:text-gray-400">kcal/den pro hubnutí</span>
          </div>
        </div>
      </div>
      ...
      {/* Projection & Stats */}
      {projection && (
        <div className="glass p-4 sm:p-6 animate-slide-up" style={{ animationDelay: "0.2s" }}>
          <h3 className="text-base sm:text-lg font-semibold text-gray-900 dark:text-gray-100 mb-4">Projekce & Statistiky</h3>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="stat-card">
                ...
                <span className="text-xs text-gray-600 dark:text-gray-400">{projection.idealTrend.totalDays} dní celkem</span>
              </div>
              <div className="stat-card">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="stat-card">
                ...
                <span className={`stat-value text-xl ${projection.currentTrend.avgDailyLoss > 0 ? "text-emerald-500 dark:text-emerald-400" : "text-red-400"}`}>
                  {projection.currentTrend.avgDailyLoss > 0 ? "-" : "+"}
              ...
              <div className="stat-card">
                ...
                <span className={`stat-value text-xl ${projection.stats.avgWeeklyLoss > 0 ? "text-emerald-500 dark:text-emerald-400" : "text-red-400"}`}>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="stat-card">
                ...
                <span className="stat-value text-xl text-emerald-500 dark:text-emerald-400">
                  {projection.stats.currentStreak} dní
                </span>
                <span className="text-xs text-gray-600 dark:text-gray-400">v řadě dolů</span>
              </div>
              <div className="stat-card">
            ...
            <div className="mt-4 pt-4 border-t border-gray-300 dark:border-gray-800">
              ...
              <div className="space-y-2">
                {[
                  { deficit: 300, label: "300 kcal/den" },
                  { deficit: 500, label: "500 kcal/den (aktuální)" },
                  { deficit: 750, label: "750 kcal/den", hideOnMobile: true },
                  { deficit: 1000, label: "1000 kcal/den", hideOnMobile: true },
                ].map((scenario) => {
                  ...
                  return (
                    <div
                      ...
                      className={`flex items-center justify-between p-2 sm:p-3 rounded-lg ${
                        scenario.deficit === 500 ? "bg-emerald-500/10 border border-emerald-500/20" : "bg-gray-200/30 dark:bg-gray-800/30"
                      } ${scenario.hideOnMobile ? "hidden sm:flex" : "flex"}`}
                    >
                      <span className={`text-xs sm:text-sm ${scenario.deficit === 500 ? "text-emerald-500 dark:text-emerald-400 font-medium" : "text-gray-600 dark:text-gray-400"}`}>
                        {scenario.label}
                      </span>
                      <div className="text-right">
                        <div className={`text-xs sm:text-sm font-semibold ${scenario.deficit === 500 ? "text-emerald-500 dark:text-emerald-400" : "text-gray-700 dark:text-gray-300"}`}>
                          {daysNeeded} dní
                        </div>
